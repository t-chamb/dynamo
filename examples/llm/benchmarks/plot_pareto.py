# SPDX-FileCopyrightText: Copyright (c) 2025 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
# http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

import json
import os
import re

import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import seaborn as sns
from matplotlib.ticker import MultipleLocator


def get_genai_perf_profile_export_json_paths(search_paths):
    paths = []
    print(f"Searching {search_paths=}")
    for search_path in search_paths:
        print(f"=== searching {search_path=}")
        for root, dirs, files in os.walk(search_path):
            print(f"=== searching {root=}, {dirs=}")
            for file in files:
                if file == "profile_export_genai_perf.json":
                    print(f"Found gap export: {file}")
                    paths.append(os.path.join(root, file))
    return paths


# search for -concurrency<number> in the name
def parse_concurrency(name):
    matches = re.findall(r"-concurrency(\d+)", name)
    if len(matches) != 1:
        raise Exception(f"non-unique matches: {matches}")
    concurrency = 0
    for c in matches:
        concurrency += int(c)
    return concurrency


# serach for _tp<number>dp<number> in the name
def parse_gpus(name):
    print(f"Parsing {name} _tp$dp$ matches...")
    matches = re.findall(r"_tp(\d+)dp(\d+)", name)
    if len(matches) <= 0 or len(matches) > 2:
        raise Exception(f"invalid matches: {matches}")
    total_gpus = 0
    for tp, dp in matches:
        total_gpus += int(tp) * int(dp)
    return total_gpus


def extract_val_and_concurrency(genai_perf_profile_export_json_paths, stat_value="avg"):
    results = []
    print(f"{genai_perf_profile_export_json_paths=}")
    for json_path in genai_perf_profile_export_json_paths:
        with open(json_path, "r") as f:
            data = json.load(f)
            # output_token_throughput contains only avg
            output_token_throughput = data.get("output_token_throughput", {}).get("avg")
            output_token_throughput_per_request = data.get(
                "output_token_throughput_per_request", {}
            ).get(stat_value)
            time_to_first_token = data.get("time_to_first_token", {}).get(stat_value)
            inter_token_latency = data.get("inter_token_latency", {}).get(stat_value)
            # request_throughput contains only avg
            request_throughput = data.get("request_throughput", {}).get("avg")
            osl = data.get("output_sequence_length", {}).get("avg")
            isl = data.get("input_sequence_length", {}).get("avg")
            # NOTE: In an attempt to better align with TRTLLM benchmarks, I updated this x-axis
            # from genai-perf's custom calculated token_throughput_per_user to just 1 / avg_ITL.
            # This may need to be re-visited, but is more intuitive when looking at the reported ITL results.
            toks_per_sec_per_user = 1 / (inter_token_latency / 1000.0)

        concurrency = parse_concurrency(json_path)
        num_gpus = parse_gpus(json_path)

        # Handle the case of num_gpus=0 to avoid division by zero
        if num_gpus > 0 and output_token_throughput is not None:
            output_token_throughput_per_gpu = output_token_throughput / num_gpus
        else:
            output_token_throughput_per_gpu = 0.0

        if num_gpus > 0 and request_throughput is not None:
            request_throughput_per_gpu = request_throughput / num_gpus
        else:
            request_throughput_per_gpu = 0.0

        results.append(
            {
                "configuration": json_path,
                "num_gpus": num_gpus,
                "concurrency": float(concurrency),
                "output_token_throughput_avg": output_token_throughput,
                f"output_token_throughput_per_request_{stat_value}": output_token_throughput_per_request,
                "output_token_throughput_per_gpu_avg": output_token_throughput_per_gpu,
                f"time_to_first_token_{stat_value}": time_to_first_token,
                f"inter_token_latency_{stat_value}": inter_token_latency,
                "request_throughput_per_gpu_avg": request_throughput_per_gpu,
                "isl": isl,
                "osl": osl,
                "toks_per_sec_per_user": toks_per_sec_per_user,
            }
        )
    return results


def create_pareto_graph(results, title="", stat_value="avg"):
    data_points = [
        {
            "label": result["configuration"].split("/")[0],
            "configuration": result["configuration"],
            "concurrency": float(result["concurrency"]),
            f"output_token_throughput_per_request_{stat_value}": result[
                f"output_token_throughput_per_request_{stat_value}"
            ],
            "output_token_throughput_per_gpu_avg": result[
                "output_token_throughput_per_gpu_avg"
            ],
            f"time_to_first_token_{stat_value}": result[
                f"time_to_first_token_{stat_value}"
            ],
            f"inter_token_latency_{stat_value}": result[
                f"inter_token_latency_{stat_value}"
            ],
            "is_pareto_efficient": False,
            "isl": result["isl"],
            "osl": result["osl"],
            "output_token_throughput_avg": result["output_token_throughput_avg"],
            "toks_per_sec_per_user": result["toks_per_sec_per_user"],
        }
        for result in results
    ]
    print(f"Results:\n{results}")
    df = pd.DataFrame(data_points)
    print(f"Dataframe:\n{df}")

    def pareto_efficient(ids, points):
        """
        Mark Pareto-efficient points.
        A point p is dominated if there's another q
        such that q is >= p in all dimensions.
        """
        points = np.array(points)
        pareto_points = []
        for i, (point_id, point) in enumerate(zip(ids, points)):
            dominated = False
            for j, other_point in enumerate(points):
                if i != j and all(other_point >= point):
                    dominated = True
                    break
            if not dominated:
                pareto_points.append(point)
                df.at[point_id, "is_pareto_efficient"] = True
        return np.array(pareto_points)

    sns.set(style="whitegrid")
    fig, ax = plt.subplots(figsize=(14, 6), constrained_layout=True)

    labels = df["label"].unique()

    for label in labels:
        group = df[df["label"] == label]
        # Scatter all points
        ax.scatter(
            # group[f"output_token_throughput_per_request_{stat_value}"],
            group["toks_per_sec_per_user"],
            group["output_token_throughput_per_gpu_avg"],
            label=f"Label {label}",
        )

        # Identify and mark Pareto frontier
        pareto_points = pareto_efficient(
            group.index,
            group[
                [
                    # NOTE: In an attempt to better align with TRTLLM benchmarks, I updated this x-axis
                    # from genai-perf's custom calculated token_throughput_per_user to just 1 / avg_ITL.
                    # This may need to be re-visited, but is more intuitive when looking at the reported ITL results.
                    # f"output_token_throughput_per_request_{stat_value}",
                    "toks_per_sec_per_user",
                    "output_token_throughput_per_gpu_avg",
                ]
            ].values,
        )
        # Sort by x-value for a clean line
        pareto_points = pareto_points[np.argsort(pareto_points[:, 0])]
        ax.plot(
            pareto_points[:, 0],
            pareto_points[:, 1],
            linestyle="--",
            label=f"Pareto Frontier {label}",
        )

    # Save CSV
    if stat_value == "avg":
        df_file_name = "results.csv"
    else:
        df_file_name = f"results_{stat_value}.csv"
    df.to_csv(df_file_name)

    # Axis labels and tick intervals
    # ax.set_xlabel(f"tokens/s/user {stat_value}")
    ax.set_xlabel(f"tokens/s/user {stat_value} (1/ITL)")
    ax.set_ylabel("tokens/s/gpu avg")
    ax.set_title(f"Pareto - {title}")
    ax.legend(bbox_to_anchor=(1.02, 1), loc="upper left")

    ax.grid(True)
    x_interval = 5
    y_interval = 5
    ax.xaxis.set_major_locator(MultipleLocator(x_interval))
    ax.yaxis.set_major_locator(MultipleLocator(y_interval))

    if stat_value == "avg":
        file_name = "pareto_plot.png"
    else:
        file_name = f"pareto_plot_{stat_value}.png"
    plt.savefig(file_name, dpi=300)
    plt.close()


if __name__ == "__main__":
    genai_perf_profile_export_json_paths = get_genai_perf_profile_export_json_paths(
        [
            # NOTE: script currently requires the format tpXdpY in the name for
            # calculating the number of GPUs for the throughput/per/gpu.
            "0xPrefill_1xDecode_TEP4_dynamo-run_tp4dp4",
            "0xPrefill_4xDecode_TEP4_dynamo-run_tp4dp4",
            ## 4xGB200
            # "0xPrefill_4xDecode_DEP4_tp4dp4",
            # "0xPrefill_4xDecode_TEP4_tp4dp4",
            # "1xPrefill_DEP4_tp4dp1_3xDecode_TEP4_tp4dp3",
            # "2xPrefill_DEP4_tp4dp2_2xDecode_TEP4_tp4dp2",
            # "3xPrefill_DEP4_tp4dp3_1xDecode_TEP4_tp4dp1",
        ]
    )
    extracted_values = extract_val_and_concurrency(genai_perf_profile_export_json_paths)
    # FIXME - set title accordingly to your results
    create_pareto_graph(
        extracted_values,
        title="Deepseek R1 4xGB200 | DynamoTRTLLM | 8K ISL 256 OSL | FP4 Weights | FP8 KV Cache",
    )
